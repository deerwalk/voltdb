...
//            checkDuplicateReplicationListener: 'This port already exists',
    ...
    setInterval(function () {
        ...
        VdmService.GetMemberList(function(connection){
            VdmUI.displayServers(connection.Metadata['MEMBER_LISTING'])
            VdmUI.displayServerLists(VdmUI.ServerList)
        ...
        VdmUI.haltRequest = true;
            ...
            if(VdmUI.isFreshStart){
             ...
             VdmService.RecoverCluster(function(connection){
              ...
              else if (connection.Metadata['VDM_RECOVER_CLUSTER'].statusString == 'Error')
              {
                     ShowHideDatabaseStatus("failed")
                    var msg = '<p>Cluster startup did not succeed on all servers.</p>';
                    msg = msg + '<ul class="listStalled">' +
                            '<li class="pullback"><span class="cross-spacer"></span>The server ports are already used by a running database</li></ul>'
...
//             }
    ...
    var showHideShutdownDialog = function(state){
        if(state && ($('#startSuccess').is(":visible") || localStorage.isStarting ==  "true")){
            ...
            saveCookie('shutdownDialogState', true)
        } else {
            $('#divShutdownCluster').hide();
            $('#startSuccess').removeClass('databaseRg1')
            saveCookie('shutdownDialogState', false)
...
(function (window) {
    var iVdmUi = (function () {
                    ...
                    VdmService.GetDatabaseStatus(function(connection){
                        if(connection.Metadata['DATABASE_STATUS'].hasOwnProperty('dbStatus')){
                            ...
                            if(memberList.members != undefined){
                                ...
                                saveCurrentServerCookie(memberList.members[0]["id"])
        ...
        this.displayServerLists = function(serverList){
            debugger;
            var htmlList = '';
            var isServerCookieDefined = true;
            var serverId = $(this.parentElement).data('id');
            var currentServerId = VdmUI.getCurrentDbCookie();
            if(currentServerId == -1){
                isServerCookieDefined = false
                currentServerId = serverList[0]['id'];
                VdmUI.saveCurrentServerCookie(currentServerId);
            }
            if(serverList ==  undefined || serverList.length == 0){
                $('#selectedServerName').html('');
                $('#warningMsg').html('No server has been added.')
                VdmUI.showHideOverlay(false);
            }else{
                VdmUI.showHideOverlay(true);
                serverList.forEach(function (info) {

                    var serverId = info['id']
                    var hostName = info["hostname"];
                    var name = info["name"]
                    var displayName =  name != '' ? name : hostName;
                    var serverInfo = JSON.stringify(info);

                    if (serverId == currentServerId){

                        $('#selectedServerName').html(displayName);
                        htmlList += '<tr>' +
                                    '<td data-id="' + serverId + '"><a id="dbInfo_' + serverId + '" class="btnServerList selected" >' + displayName + '</a></td>' +
                                    '</tr>';
                    }
                    else{
                         htmlList += '<tr>' +
            ...
            if(htmlList == ""){
                $('#serverList1').html('<tr><td style="top:-1px !important">No servers available.</td><td></td></tr>')
            }else{
                $('#serverList1').html(htmlList)
            }

            $('.btnServerList').on('click', function(){
                debugger;
                VdmUI.saveCurrentServerCookie($(this.parentElement).data('id'));
                $(this).addClass('selected');

                $('#selectedServerName').html($(this).html());
                var dbData = {
        ...
        this.saveCurrentServerCookie = function(serverId){
             saveCookie("current-server", serverId);
        };
        ...
        this.getCurrentServerCookie = function(){
            ...
            var count = 0;
            try {
                serverId = $.parseJSON(VdmUI.getCookie("current-server"));
            } catch (e) {
                //do nothing
            }
            return serverId;
...
//                $("#txtJdbcUrlValue").val("")
        ...
        this.displaySecurity = function(){
            ...
            VdmService.GetDeploymentUsers(function(connection){
                VdmUI.DeploymentUsers = connection.Metadata['DEPLOYMENT_USER']
                for(var i = 0; i < VdmUI.DeploymentUsers.users.length; i++){

                    var username = VdmUI.DeploymentUsers.users[i].name;
                    VdmUI.orgUserList.push(username);
                    VdmUI.userId = VdmUI.DeploymentUsers.users[i].userid;
                    var roles = VdmUI.DeploymentUsers.users[i].roles;
                        securityList += '<tr  style="display:'+showHide+'" class="child-row-6 subLabelRow securityList" data-id=\''+ VdmUI.userId +'\' data-property=\''+ JSON.stringify(VdmUI.DeploymentUsers.users[i]) +'\'><td class="configLabel">'+ username + '</td><td>'+ roles +'</td><td>&nbsp;</td><td>' +
                ...
                if(!(connection.Metadata['DATABASE_STATUS'].dbStatus.status=="stalled" && VdmUI.isRunning) && isStarting == "true")
                  ...
                  if (connection.Metadata['DATABASE_STATUS'].statusString == "No Members"){
                 ...
                 else if (connection.Metadata['DATABASE_STATUS'].dbStatus.status=="running")
                    {
                        ...
                        VdmUI.ShowHideDatabaseStatus("running", isStarting)
