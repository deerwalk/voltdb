...
var loadPage = function() {
    ...
    $('#chkSNMP').on('ifClicked', function(event){
        if($('#onOffSnmp').text()=="Off"){
            $('#onOffSnmp').text("On")
        }
        else if ($('#onOffSnmp').text()=="On"){
            $('#onOffSnmp').text("Off");
            $('#errorAuthKey').hide();
            $('#errorPrivacyKey').hide();
        }
    });

    $( "#selectAuthProtocol" ).change(function() {
        var isEnabled = false;
        if($($('#chkSNMP').parent()).hasClass('checked'))
            isEnabled = true;
        VdmUI.addRemoveAuthkeyValidation(clusterObjects.txtUsername.val(), clusterObjects.selectAuthProtocol.val(), isEnabled)
    });

    $( "#selectPrivacyProtocol" ).change(function() {
        var isEnabled = false;
        if($($('#chkSNMP').parent()).hasClass('checked'))
            isEnabled = true;
        VdmUI.addRemovePrivacyKeyValidation(clusterObjects.txtUsername.val(), clusterObjects.selectPrivacyProtocol.val(), isEnabled)
    });
    ...
    $('#chkSNMP').on('ifClicked', function(event){
        ...
        if(isEnabled){
            $('#txtTarget').rules('add', {
                required: true,
                ipPortRegex: /^[a-zA-Z0-9.-]+$/,
                messages: {
                    required: "This field is required.",
                ipPortRegex: 'Please enter a valid target.'
                }
            });

            VdmUI.addRemoveAuthkeyValidation(clusterObjects.txtUsername.val(), clusterObjects.selectAuthProtocol.val(), true)
            VdmUI.addRemovePrivacyKeyValidation(clusterObjects.txtUsername.val(), clusterObjects.selectPrivacyProtocol.val(), true)
        } else {
            $('#txtTarget').rules('remove');
            $('#txtAuthKey').rules('remove');
            $('#txtPrivacyKey').rules('remove');
            $('#errorTarget').hide();
            $('#errorAuthKey').hide();
            $('#errorPrivacyKey').hide();
        }
    ...
    $('#txtUsername').on('input propertychange paste', function() {
        var isEnabled = false;
        if($($('#chkSNMP').parent()).hasClass('checked'))
            isEnabled = true;
        VdmUI.addRemoveAuthkeyValidation(clusterObjects.txtUsername.val(), clusterObjects.selectAuthProtocol.val(), isEnabled);
...
//            checkDuplicateReplicationListener: 'This port already exists',
    ...
    clusterObjects = {
        ...
        selectAuthProtocol : $('#selectAuthProtocol'),
        txtAuthKey: $('#txtAuthKey'),
        selectPrivacyProtocol: $('#selectPrivacyProtocol'),
        txtPrivacyKey: $('#txtPrivacyKey'),
        chkSNMP: $('#chkSNMP'),
        chkSNMPOnOffVal: $('#chkSNMPOnOffVal'),
    }
  ...
  $("#startOption").attr('checked',false)
    ...
    $('#frmSNMPConfig').validate({
        rules:{
            txtTarget: validationRules.IpPortRule
        },
        messages:{
            txtTarget: validationRules.IpPortMessage
        }
    })
...
//                }
    ...
    $('#btnSaveReplication').on('click', function(e){
        ...
        if(clusterObjects.selectDrRole.val() == 'replica' || clusterObjects.selectDrRole.val() == 'xdcr'){
            if(!$('.trConnectionSource').is(':visible') || clusterObjects.txtDatabase.val().trim() == ''){
                $('#drServers-error').show()
                $('#drServers-error').html('Connection source must be defined for replica and xdcr role.')
                return false
            } else {
                $('#drServers-error').hide()
            }
    ...
    clusterObjects.txtDatabase.on("input", function(){
        VdmUI.showHideConnectionErrMsg()
    })

    clusterObjects.selectDrRole.on("change", function(){
        VdmUI.showHideConnectionErrMsg()
    })
    ...
    var SaveDrInfo = function(){
        ...
        if($('.trConnectionSource').is(':visible') && masterCluster.trim() != ''){
            connection_source = {
        ...
        if(clusterObjects.txtDrPort.val() != ''){
            dr_value = {
                ...
                role: clusterObjects.selectDrRole.val(),
            }
        } else {
            dr_value = {
                id: parseInt(clusterObjects.txtDrId.val()),
    ...
    $('#btnSaveSNMP').on('click', function(e){
        if(!$('#frmSNMPConfig').valid()){
            e.preventDefault();
            e.stopPropagation();
            return;
        }
    ...
    var SaveSNMPConfig = function(){
        ...
        snmp_value = {
            ...
            community: $('#txtCommunity').val(),
            username: $('#txtUsername').val(),
            authprotocol: $('#selectAuthProtocol').val(),
            authkey: $('#txtAuthKey').val(),
            privacyprotocol: $('#selectPrivacyProtocol').val(),
            privacykey: $('#txtPrivacyKey').val()
        }
    ...
    $('#btnUpdateSNMP').on('click',function(){
        $('#SNMPConfig').modal('show');
        $('#errorTarget').hide()
        $('#errorAuthKey').hide();
        $('#errorPrivacyKey').hide();
        VdmUI.populateSNMPPopup();
    });

    $('#btnDeleteSNMP').on('click', function(){
        $('#SNMPConfig').modal('hide');
        deploymentInfo = {
                            data: {
                                snmp: {
                                }
...
(function (window) {
    var iVdmUi = (function () {
        ...
        this.showHideConnectionErrMsg = function(){
            if(clusterObjects.selectDrRole.val() == 'replica' || clusterObjects.selectDrRole.val() == 'xdcr'){
                if(!$('.trConnectionSource').is(':visible') || clusterObjects.txtDatabase.val().trim() == ''){
                    $('#drServers-error').show()
                    $('#drServers-error').html('Connection source must be defined for replica and xdcr role.')
        ...
        this.displayDeployment = function(deployment){
            ...
            if(deployment['deployment']['dr'] != null && !$.isEmptyObject(deployment['deployment']['dr'])){
                ...
                var role = deployment['deployment']['dr'].role
                clusterObjects.lblDrRole.text(role == 'xdcr' ? role.toUpperCase() : VdmUI.capitalizeFirstLetter(role))
            ...
            //SNMP Configuration
            if(deployment['deployment']['snmp'] != null && !$.isEmptyObject(deployment['deployment']['snmp'])){
                if(deployment['deployment']['snmp'].enabled == true || deployment['deployment']['snmp'].enabled == 'true') {
                    $('#onOffIconSNMP').addClass('checked')
                    clusterObjects.chkSNMPOnOffVal.text('On')
                } else {
                    $('#onOffIconSNMP').removeClass('checked')
                    clusterObjects.chkSNMPOnOffVal.text('Off')
                }
                $('#tdTargetVal').text(deployment['deployment']['snmp'].target);
                $('#tdCommunityVal').text(deployment['deployment']['snmp'].community);
                $('#tdUsernameVal').text(deployment['deployment']['snmp'].username);
                $('#tdAuthProtocolVal').text(deployment['deployment']['snmp'].authprotocol);
                $('#tdAuthKeyVal').text(deployment['deployment']['snmp'].authkey);
                $('#tdPrivacyProtocolVal').text(deployment['deployment']['snmp'].privacyprotocol);
                $('#tdPrivacyKeyVal').text(deployment['deployment']['snmp'].privacykey);
            } else {
                $('#onOffIconSNMP').removeClass('checked')
                clusterObjects.chkSNMPOnOffVal.text('Off')
                $('#tdTargetVal').text('');
                $('#tdCommunityVal').text('');
                $('#tdUsernameVal').text('');
                $('#tdAuthProtocolVal').text('');
                $('#tdAuthKeyVal').text('');
                $('#tdPrivacyProtocolVal').text('');
                $('#tdPrivacyKeyVal').text('');
            }
...
//                $("#txtJdbcUrlValue").val("")
        ...
        this.populateDrPopup = function(){
            ...
            VdmService.GetDeployment(function(connection){
                var deploymentInfo = connection.Metadata['DEPLOYMENT'];
                clusterObjects.selectDrRole.val('master');
                if(deploymentInfo['deployment']['dr'] != undefined && !$.isEmptyObject(deploymentInfo['deployment']['dr']))
                {
                    ...
                    clusterObjects.selectDrRole.val(deploymentInfo['deployment']['dr'].role != undefined ? deploymentInfo['deployment']['dr'].role: 'master')
        ...
        this.populateSNMPPopup = function(){
            dbData = {
               id: VdmUI.getCurrentDbCookie()
            }
            VdmService.GetDeployment(function(connection){
                var deploymentInfo = connection.Metadata['DEPLOYMENT'];
                if(deploymentInfo['deployment']['snmp'] != undefined && !$.isEmptyObject(deploymentInfo['deployment']['snmp'])){
                    if(deploymentInfo['deployment']['snmp']['enabled'] == true){
                        $('#chkSNMP').iCheck('check')
                        $('#onOffSnmp').text('On')
                        ...
                        VdmUI.addRemoveAuthkeyValidation(deploymentInfo['deployment']['snmp'].username, deploymentInfo['deployment']['snmp'].authprotocol, true)
                        VdmUI.addRemovePrivacyKeyValidation(deploymentInfo['deployment']['snmp'].username, deploymentInfo['deployment']['snmp'].privacyprotocol, true)
                    } else {
                        $('#txtTarget').rules('remove');
                        $('#txtAuthKey').rules('remove');
                        $('#txtPrivacyKey').rules('remove');
                        $('#chkSNMP').iCheck('uncheck')
                        $('#onOffSnmp').text('Off')
                    }


                    clusterObjects.txtTarget.val(deploymentInfo['deployment']['snmp'].target);
                    clusterObjects.txtCommunity.val((deploymentInfo['deployment']['snmp'].community == undefined ||
                                                    deploymentInfo['deployment']['snmp'].community == '') ? 'public' :
                                                    deploymentInfo['deployment']['snmp'].community)
                    clusterObjects.txtUsername.val(deploymentInfo['deployment']['snmp'].username)
                    clusterObjects.selectAuthProtocol.val((deploymentInfo['deployment']['snmp'].authprotocol == undefined ||
                                                    deploymentInfo['deployment']['snmp'].authprotocol == '') ? 'SHA' :
                                                    deploymentInfo['deployment']['snmp'].authprotocol)
                    clusterObjects.txtAuthKey.val((deploymentInfo['deployment']['snmp'].authkey == undefined ||
                                                    deploymentInfo['deployment']['snmp'].authkey == '') ? 'voltdbauthkey' :
                                                    deploymentInfo['deployment']['snmp'].authkey)
                    clusterObjects.selectPrivacyProtocol.val((deploymentInfo['deployment']['snmp'].privacyprotocol == undefined ||
                                                    deploymentInfo['deployment']['snmp'].privacyprotocol == '') ? 'AES' :
                                                    deploymentInfo['deployment']['snmp'].privacyprotocol)
                    clusterObjects.txtPrivacyKey.val((deploymentInfo['deployment']['snmp'].privacykey == undefined ||
                                                    deploymentInfo['deployment']['snmp'].privacykey == '') ? 'voltdbprivacykey' :
                                                    deploymentInfo['deployment']['snmp'].privacykey)
                } else {
                    $('#chkSNMP').iCheck('uncheck')
                    $('#onOffSnmp').text('Off')
                    $('#txtTarget').rules('remove');
                    $('#txtAuthKey').rules('remove');
                    $('#txtPrivacyKey').rules('remove');
                    clusterObjects.txtTarget.val('')
                    clusterObjects.txtCommunity.val('public')
                    clusterObjects.txtUsername.val('')
                    clusterObjects.selectAuthProtocol.val('SHA')
                    clusterObjects.txtAuthKey.val('voltdbauthkey')
                    clusterObjects.selectPrivacyProtocol.val('AES')
                    clusterObjects.txtPrivacyKey.val('voltdbprivacykey')
                }
            },dbData);
        }
        ...
        this.addRemoveAuthkeyValidation = function(username, authProtocol, isSNMPEnabled){
            if(isSNMPEnabled && authProtocol != 'NoAuth'){
                $('#txtAuthKey').rules('remove' );
        ...
        this.addRemovePrivacyKeyValidation = function(username, privacyProtocol, isSNMPEnabled){
            if(isSNMPEnabled  && privacyProtocol != 'NoPriv'){
                $('#txtPrivacyKey').rules('remove');
                if(username != undefined && username != ''){
                    $('#txtPrivacyKey').rules('add', {
                        required: true,
                        minlength: 8,
                        messages: {
                            required: 'This field is required.',
